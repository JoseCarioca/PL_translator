...
#   | PRINTF '(' CADENA ',' AuxPrintf ')' | SCANF '(' CADENA_SCANF ','  AuxScanf ')' | PRINTF '(' CADENA_SCANF, AuxPrintf ')'
#
...
#   Declaracion3 -> ID | ID CORCHETES | ID '=' Operation | ID CORCHETES '=' Operation 
#
#   TIPO_ID -> TIPO ID
#   TIPO -> INT
...
class P1Parser(Parser):
    ...
    def Declaracion(self,p):
        ...
        return [(p.TIPO_ID[0]+len(p.CORCHETES)*"*", p.TIPO_ID[1], p.CORCHETES, -self.ebp)]
    ...
    def fact(self,p):
        ...
        for i in range(0,2):
            for j in range(0,6):
                print(str([i,j]) + str(linear_index([2,6],[i,j])))
        #print(linear_index([3,4],[0,3]))
        var = self.Variables.get((p.ID,self.current_function)) or self.Variables.get((p.ID,"Global"))
        if var == None:
            print("Error: Variable "+p.ID+" no declarada")
            self.ErrorFlag = True
...
def linear_index(dims, subindex):
    """
        dims (list): Lista de Dimension de array
        subindex (list): Lista de indices pedidos
