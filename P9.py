...
class P1Parser(Parser):
    ...
    def Declaracion(self,p):
        ...
        if (p.TIPO_ID[1],self.current_function) in self.Variables.keys():
            print("Variable '" + p.TIPO_ID[1] + "' ya declarada en '" + self.current_function + "' previamente")
            self.ErrorFlag = True
        else:
            self.Variables[(p.TIPO_ID[1],self.current_function)] = varAux(p.TIPO_ID[0],[1],-self.ebp)
    ...
    def Declaracion(self,p):
        ...
        if (p.TIPO_ID[1],self.current_function) in self.Variables.keys():
            ...
            self.Variables[(p.TIPO_ID[1],self.current_function)] = varAux(p.TIPO_ID[0]+len(p.CORCHETES)*"*", p.CORCHETES, -arrayEBP)
    ...
    def Declaracion3(self,p):
        ...
        if (p.ID,self.current_function) in self.Variables.keys():
            print("Variable '" + p.ID + "' ya declarada en '" + self.current_function + "' previamente")
            self.ErrorFlag = True
        else:
            self.Variables[(p.ID,self.current_function)] = varAux(self.current_tipo,[1],-self.ebp)
    ...
    def Declaracion3(self,p):
        ...
        if (p.ID,self.current_function) in self.Variables.keys():
            ...
            self.Variables[(p.ID,self.current_function)] = varAux(self.current_tipo+len(p.CORCHETES)*"*", p.CORCHETES, -arrayEBP)

        self.Traduccion += "\n\tsubl $" + str(reserva) + ", %esp\n"
        return [(self.current_tipo+len(p.CORCHETES)*"*",p.ID,p.CORCHETES,-arrayEBP)]
    ...
    def fact(self,p):
        ...
        if not self.ErrorFlag:
            ...
            if flagGlobal:
                self.Traduccion += "\n"
                self.Traduccion += "\tpushl $" + p.ID + str(pos) + "\n"
            else:
